 ...
 *
 * With some modifications for vk-graph-export (fixes)
 */
		...
		do{
			...
			if( typeof child == 'string' ){
				if( nc == 1 )//single text node
					return buffer.push( buffer.pop() + escape(child) + '</'+node.n+'>' );					
				else //regular text node
					buffer.push( indent+chr+escape(child) );
